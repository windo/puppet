...
class Puppet::SSLCertificates::CA
    ...
    def removeclientcsr(host)
        csrfile = host2csrfile(host)
        raise Puppet::Error, "No certificate request for %s" % host unless File.exists?(csrfile)
    ...
    def setconfig(hash)
        ...
        raise ArgumentError, "Unknown parameters %s" % hash.keys.join(",") if hash.length > 0

        [:cadir, :csrdir, :signeddir].each { |dir|
            raise Puppet::DevError, "%s is undefined" % dir unless @config[dir]
        }
    end
    ...
    def storeclientcert(cert)
        ...
        certfile = host2certfile(host)
        Puppet.notice "Overwriting signed certificate %s for %s" % [certfile, host] if File.exists?(certfile)
