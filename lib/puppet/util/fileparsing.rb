...
module Puppet::Util::FileParsing
    ...
    class FileRecord
        ...
        def fields=(fields)
            @fields = fields.collect do |field|
                r = symbolize(field)
                raise ArgumentError.new("Cannot have fields named %s" % r) if INVALID_FIELDS.include?(r)
                r
            end
        end
        ...
        def initialize(type, options = {}, &block)
            @type = symbolize(type)
            raise ArgumentError, "Invalid record type %s" % @type unless [:record, :text].include?(@type)
    ...
    def new_line_type(record)
        ...
        raise ArgumentError, "Line type %s is already defined" % record.name if @record_types.include?(record.name)
