...
class Puppet::Util::Settings::Setting
    ...
    def initialize(args = {})
        ...
        args.each do |param, value|
            method = param.to_s + "="
            raise ArgumentError, "%s does not accept %s" % [self.class, param] unless self.respond_to? method

            self.send(method, value)
        end

        raise ArgumentError, "You must provide a description for the %s config option" % self.name unless self.desc
    end
    ...
    def to_config
        ...
        str += "# The default value is '%s'.\n" % @default if defined? @default and @default
