...
class Puppet::Util::Feature
    ...
    def add(name, options = {})
        method = name.to_s + "?"
        raise ArgumentError, "Feature %s is already defined" % name if self.class.respond_to?(method)
    ...
    def load_library(lib, name)
        raise ArgumentError, "Libraries must be passed as strings not %s" % lib.class unless lib.is_a?(String)
