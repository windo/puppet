...
class Puppet::Util::Settings
    ...
    def setdefaults(section, defs)
        ...
        defs.each { |name, hash|
            ...
            raise ArgumentError, "Parameter %s is already defined" % name if @config.include?(name)
...
if @config.include?(:name)
    ...
    def readwritelock(default, *args, &bloc)
        ...
        tmpfile = file + ".tmp"
        sync = Sync.new
        raise Puppet::DevError, "Cannot create %s; directory %s does not exist" % [file, File.dirname(file)] unless FileTest.directory?(File.dirname(tmpfile))
    ...
    def get_config_file_default(default)
        ...
        raise ArgumentError, "Default %s is not a file" % default unless obj.is_a? FileSetting
        ...
        value = string.sub(/\{\s*([^}]+)\s*\}/) do
            ...
            params.split(/\s*,\s*/).each do |str|
                if str =~ /^\s*(\w+)\s*=\s*([\w\d]+)\s*$/
                    ...
                    result[param] = value
                    raise ArgumentError, "Invalid file option '%s'" % param unless [:owner, :mode, :group].include?(param)
