module Puppet
    newtype(:exec) do
        ...
        def checkexe(cmd)
            if cmd =~ /^\//
                exe = cmd.split(/ /)[0]
                raise ArgumentError, "Could not find executable %s" % exe unless FileTest.exists?(exe)
        ...
        def validatecmd(cmd)
            ...
            if cmd !~ /^\//
                self.fail "'%s' is both unqualifed and specified no search path" % cmd if self[:path].nil?
            end
        end
    end
end
