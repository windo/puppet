    ...
    newparam(:range) do
      ...
      munge do |values|
        ...
        values.each { |value|
          ...
          self.fail "Invalid range %s" % value if range.length != 2

          # Make sure the hours are valid
          [range[0][0], range[1][0]].each do |n|
            raise ArgumentError, "Invalid hour '%s'" % n if n < 0 or n > 23
          end

          [range[0][1], range[1][1]].each do |n|
            raise ArgumentError, "Invalid minute '%s'" % n if n and (n < 0 or n > 59)
          end
