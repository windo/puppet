...
class Rights
    ...
    class Right < Puppet::Network::AuthStore
        ...
        def restrict_method(m)
            ...
            raise ArgumentError, "'%s' is not an allowed value for method directive" % m unless ALL.include?(m)
            ...
            raise ArgumentError, "'%s' is already in the '%s' ACL" % [m, name] if @methods.include?(m)
        ...
        def restrict_environment(env)
            ...
            raise ArgumentError, "'%s' is already in the '%s' ACL" % [env, name] if @environment.include?(env)
