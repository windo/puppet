...
Puppet::Type.type(:package).provide :pkgdmg, :parent => Puppet::Provider::Package do
  ...
  def self.installpkgdmg(source, name)
    ...
    begin
      File.open(cached_source) do |dmg|
        ...
        raise Puppet::Error.new("No disk entities returned by mount at %s" % dmg.path) unless hdiutil_info.has_key?("system-entities")
  ...
  def query
    return FileTest.exists?("/var/db/.puppet_pkgdmg_installed_#{@resource[:name]}")?{:name => @resource[:name], :ensure => :present}:nil
  end
