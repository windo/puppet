...
class DirectoryService < Puppet::Provider::NameService
    ...
    def self.get_macosx_version_major
        ...
        begin
            ...
            fail("%s is not supported by the directoryservice provider" % product_version_major) if %w{10.0 10.1 10.2 10.3}.include?(product_version_major)
    ...
    def self.generate_attribute_hash(input_hash, *type_properties)
        ...
        attribute_hash[:password] = self.get_password(attribute_hash[:guid]) if @resource_type.validproperties.include?(:password)
    ...
    def self.get_password(guid)
        ...
        if File.exists?(password_hash_file) and File.file?(password_hash_file)
            fail("Could not read password hash file at #{password_hash_file} for #{@resource[:name]}") if not File.readable?(password_hash_file)
