...
class Puppet::Provider::NameService < Puppet::Provider
    class << self
        ...
        def options(name, hash)
            raise Puppet::DevError, "%s is not a valid attribute for %s" % [name, resource_type.name] unless resource_type.valid_parameter?(name)
            @options ||= {}
            @options[name] ||= {}
        ...
        def validate(name, value)
            ...
            if @checks.include? name
                ...
                raise ArgumentError, "Invalid value %s: %s" % [value, @checks[name][:error]] unless block.call(value)
            end
        end
