...
class Type
  ...
  def self.copyparam(klass, name)
    ...
    raise Puppet::DevError, "Class %s has no param %s" % [klass, name] unless param
    @parameters << param
  ...
  def self.newproperty(name, options = {}, &block)
    ...
    raise Puppet::DevError, "Class %s already has a property named %s" % [self.name, name] if @validproperties.include?(name)
  ...
  def self.validparameter?(name)
    raise Puppet::DevError, "Class %s has not defined parameters" % self unless defined?(@parameters)
  ...
  def [](name)
    ...
    fail("Invalid parameter %s(%s)" % [name, name.inspect]) unless self.class.validattr?(name)
  ...
  def []=(name,value)
    ...
    fail("Invalid parameter %s" % [name]) unless self.class.validattr?(name)

    if name == :name
      name = self.class.namevar
    end
    raise Puppet::Error.new("Got nil value for %s" % name) if value.nil?
  ...
  def newattr(name, options = {})
    ...
    raise Puppet::Error, "Parameter '%s' is already defined in %s" % [name, self.ref] if @parameters.include?(name)
  ...
  def self.[]=(name,object)
    ...
    if exobj = @objects[name] and self.isomorphic?
      ...
      msg += ("in file %s at line %s" % [object.file, object.line]) if exobj.file and exobj.line
      msg += ("and cannot be redefined in file %s at line %s" % [object.file, object.line]) if object.file and object.line
  ...
  def self.instances
    raise Puppet::DevError, "%s has no providers and has not overridden 'instances'" % self.name if provider_hash.empty?
  ...
  newmetaparam(:check) do
    ...
    munge do |args|
      ...
      self.devfail "No parent for %s, %s?" % [self.class, self.name] unless defined?(@resource)
