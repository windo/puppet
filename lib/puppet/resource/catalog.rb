...
class Puppet::Resource::Catalog < Puppet::SimpleGraph
    ...
    def add_resource(*resources)
        resources.each do |resource|
            raise ArgumentError, "Can only add objects that respond to :ref, not instances of %s" % resource.class unless resource.respond_to?(:ref)
    ...
    def extraction_format=(value)
        raise ArgumentError, "Invalid extraction format %s" % value unless respond_to?("extract_to_%s" % value)
    ...
    def extract_to_transportable
        ...
        walk(main, :out) do |source, target|
            ...
            if child = target.to_trans
                raise "No bucket created for %s" % source unless bucket
                bucket.push child
    ...
    def fail_unless_unique(resource)
        ...
        msg << " in file %s at line %s" % [existing_resource.file, existing_resource.line] if existing_resource.file and existing_resource.line
