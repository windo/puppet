...
class Puppet::Parser::Resource < Puppet::Resource
    ...
    def override_parameter(param)
        ...
        unless param.source.child_of?(current.source)
            ...
            msg += " by %s" % current.source if current.source.to_s != ""
    ...
    def extract_parameters(params)
        params.each do |param|
            ...
            self.fail Puppet::ParseError, "Duplicate parameter '%s' for on %s" % [param.name, self.to_s] if @parameters[param.name]
