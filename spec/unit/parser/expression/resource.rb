...
describe Puppet::Parser::Expression::Resource do
  ...
  before :each do
    @title = Puppet::Parser::Expression::String.new(:value => "mytitle")
    @compiler = Puppet::Parser::Compiler.new(Puppet::Node.new("mynode"))
    @scope = Puppet::Parser::Scope.new(:compiler => @compiler)
    @scope.stubs(:resource).returns(stub_everything)
    @resource = ast::Resource.new(:title => @title, :type => "file", :parameters => ast::ArrayConstructor.new(:children => []) )
  ...
  it "should flatten the titles array" do
    ...
    %w{one two}.each do |title|
      titles << Puppet::Parser::Expression::String.new(:value => title)
    end
  ...
  describe "when generating qualified resources" do
    ...
    def resource(type, params = nil)
      params ||= Puppet::Parser::Expression::ArrayConstructor.new(:children => [])
      Puppet::Parser::Expression::Resource.new(:type => type, :title => Puppet::Parser::Expression::String.new(:value => "myresource"), :parameters => params)
    end
